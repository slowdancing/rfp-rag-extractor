...
> ⚠️ **sudo 없는 환경(부트캠프 JupyterHub VM 등)**: 위 install.sh 가 안 되면 GitHub 릴리스의
> tar 바이너리를 홈에 풀어 `~/ollama/bin`을 PATH에 추가하고 `nohup ollama serve &`로 띄운다.
> 실제 진행·트러블슈팅은 `VM_배포_학습정리.md` 참고.

## 2. 모델 받기 (확정 모델)
```bash
ollama pull exaone3.5:7.8b   # LLM (한국어 특화, 확정)
ollama pull bge-m3           # 임베딩 (한국어 강함, 확정)
```
...
python -m scripts.ask "이 사업의 주요 요구사항은?"   # EXAONE(Ollama)로 답변
python -m scripts.eval_retrieval         # 검색 평가
```

## 5-1. API 배포 (외부 접속)
```bash
nohup uvicorn src.api.main:app --host 0.0.0.0 --port 8501 > ~/api.log 2>&1 & disown
# GCP 방화벽에서 8501(TCP)을 내 IP로 개방 → 로컬에서 http://<VM외부IP>:8501 접속
```
> 웹전용·무sudo 환경이라 외부 포트를 못 열면, VM 내부 노트북(`notebooks/demo.ipynb`)으로 시연.
